...
const addAddresses = async (req, res, next) => {
  try {
    const { orderByName, contact, street, city, state, zipcode, type } =
      req.body;

    if (
      [orderByName, contact, street, city, state, zipcode, type].some(
        (field) => field?.trim() === ""
      )
    ) {
...
const deleteAddressById = async (req, res, next) => {
  try {
    ...
    const addrExists = !updatedUser.addresses.some(
      (addr) => addr._id.toString() === addressId
    );
    ...
    if (!addrExists) {
      return res
        .status(404)
        .json({ message: "Address Not Found or Already Removed" });
    }
